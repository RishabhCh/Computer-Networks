...
void processServerResponse(int *sockfd, char*fileName, int *windowSize, int *blockSize, double *fileSize, int priKey)
{
    *fileSize = 0;
    ...
    while(1)
    {
        ...
        if(msgLen > 0)
        {
            ...
            // Decrypt File
            for(int k=1; k<=msgLen - 1;k++)
            {
                decryptedByte = bbdecode((unsigned int)serverResponse[k],priKey);
                serverResponse[k] = 0x00FF & decryptedByte;
            }
...
int main(int argc, char **argv) 
{
    ...
    while(attempts++ < 5)
    {
        ...
        processServerResponse(&sockfd, fileName, &windowSize, &blockSize, &fileSize, secretKey);
