...
void processClientRequest(char *fileName,struct sockaddr_in *cliAddr, char *ipAddress, int *timeout, int *windowSize, int *blockSize, unsigned int publicKey)
{
    ...
    while(1)
    {
        ...
        for(i=0;i<(*windowSize);i++)
        {
            ...
            {
                encryptedByte = bbencode((unsigned int)windowBuffer[i][k],publicKey);
                windowBuffer[i][k] = 0x00FF & encryptedByte;
            }
    ...
    while(1)
    {
        ...
        if(msgLen > 0)
        {
            ...
            if(k==0)
            {
                ...
                processClientRequest(fileName,&cliAddr, serverIP, &timeout, &windowSize, &blockSize, publicKey);
                exit(0);
            }
        }
    }
