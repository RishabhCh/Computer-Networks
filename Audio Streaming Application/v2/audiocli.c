...
double epsilon = 0, beta = 0, alpha = 0;
...
void assignControlConstants()
{
   ...
   if(client.method == 1 || client.method == 2)
   {
	   fscanf(fptr,"%lf",&beta);
...
void updateLambda()
{
	...
	if(client.method == 2)
	{
		if(client.targetbuf > client.current_bufsize)
		{
			lambda = lambda - beta * ((lambda - alpha*gamma)*1.0);
		}
		else
		{
			lambda = lambda - beta * ((lambda - gamma)*1.0);
...
int get_server_response_to_stream(int* sockfd){
	...
	sem_destroy(&mutex);
	//deallocate_2d_array(buffer_log, 1000);
	if(error) return -1;
	return 0;
}
